        ...
        for (offset = 0; offset < ((Map.width*Map.height)*2); offset+=2)
        {
                ...
                (Field.Unit  == 0x3F)))
            {
                ...
                if (index == -1)
                {
                    ...
                    {
                        show_error("Cannot create new building in memory!");
                        return;
                    }
                    else
                    {
        ...
        if (Building_stat.num_F > 0)
        {
            for (offset = 0; offset < ((Map.width*Map.height)*2); offset+=2)
            {
                ...
                if ((Field.Part >= 0x0C) && (Field.Part <= 0xE) )
                {
                    ...
                    if (index == -1)
                    {
                        show_warning("No data for factory "+QString::number(fi)+" found!");
        ...
        if (Building_stat.num_D > 0)
        {
            for (offset = 0; offset < ((Map.width*Map.height)*2); offset+=2)
            {
                ...
                if ((Field.Part >= 0x0F) && (Field.Part <= 0x11))
                {
                    ...
                    if (index == -1)
                    {
                        show_warning("No data for depot " + QString::number(di) + " found!");
                        //Try to fix the error
                        if (Field.Part == 0x0F)
                            Bdata.Owner = 2;  //neutral
                        if (Field.Part == 0x10)
        ...
        if (Building_stat.num_T > 0)
        {
            for (offset = 0; offset < ((Map.width*Map.height)*2); offset+=2)
            {
                    ...
                    (Field.Unit == 0x3E) || (Field.Unit == 0x3F))
                {
                    ...
                    if (index == -1)
                    {
                        show_warning("No data for transport unit "+QString::number(ti)+" found! Field : "+QString::number(offset/2)+" Unit :"+QString::number(Field.Unit));
                        //Try to fix the error
                        if ((Field.Unit == 0x2C) || (Field.Unit == 0x34) || (Field.Unit == 0x3E))
                            Bdata.Owner = 0;  //german
                        else
