...
int Add_map(std::string codes_filename, QString newcode)
{
    ...
    {
        show_error("Error reloading CODES.DAT!");
        return -2;
    }
...
int Remove_map(std::string codes_filename, QString removecode)
{
    ...
    if (CODESDAT_buffer != NULL)
    {
       ...
       int remove_pos = i*10;
        ...
        if (brokencampaign)
        {
            show_warning("A map has been removed from the single-player campaign. This will lead to problems when the campaign is played, as video sequences etc. can no longer be assigned correctly.");
